 ...
 * (ResellerTableRenderer::handleMovies emits data-only rows), with the cover,
 * title, category, connections badge and per-row action dropdown all rendered
 * client-side.
 *
 ...
 * `multi` endpoint), no encode / stop / edit / delete / player (ResellerApiDispatcher
 * exposes only `action=connections&sub=purge`), no server / status / TMDB /
 * stream-info columns (the reseller movies query does not fetch that data). The only
 * row action is Kill Connections, gated by `reseller_client_connection_logs`. The
 * category filter is limited to the reseller's permitted categories
 * (`$rPermissions['category_ids']`). Permission gating uses
 * $rPermissions['can_view_vod'], not Authorization::check.
 */
...
<script>
    (function() {
        ...
        var lang = {
            ...
            confirmKill: 'Are you sure you want to kill all connections for this movie?'
        };
        ...
        var table = jQuery('#movies-table').DataTable({
            ...
            ajax: {
                url: './table',
                data: function(d) { d.id = 'movies'; d.category = document.getElementById('filter-category').value; }
            },
        ...
        jQuery('#movies-table tbody').on('click', '.js-act', function() {
