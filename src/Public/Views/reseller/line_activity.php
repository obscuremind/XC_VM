...
/**
 * Reseller Activity logs (Bootstrap 5). Full-parity port of admin/line_activity.php
 * adapted to the reseller contract: clean-JSON keyed serverSide DataTable
 * (ResellerTableRenderer::handleLineActivity emits data-only rows) with the
 * user/stream links, country flag, duration badge and IP whois rendered
 * client-side. Read-only log — no per-row actions beyond the IP whois lookup.
 *
 * Reseller differences vs admin: gated by $rPermissions (not Authorization::check);
 * no server column (resellers have no server-view access). Line + stream filters are
 * select2-ajax (reseller api userlist / streamlist); the owner filter is a static
 * select2 built from the reseller's report tree; plus a flatpickr date range.
 * Whois unwraps the reseller api's {result,data} envelope.
 */
...
// Deep-link filters (?line=ID / ?stream=ID / ?user=ID).
