...
- Hard revocation mode (`lb_revocation_mode=hard`): without a licence the extension refuses the node's session, so the `LICENCE_INVALID` its next request gets carries its pending restrictive commands (`CommandBus::restrictive`), each under its own `cmd` signature, and marks nothing delivered. Tests: `HardModeKillChannelTest` (graceful: kills on the long-poll; hard: in the denial, never a granting command, only to a node with COMMANDS).
- **Not built yet:** the `artefact` op and the agent's artefact download with its hash refusal (`ArtefactHashRefusalTest`); the agent running the commands a `LICENCE_INVALID` denial carries (XC\_VM\_Fanout); E2E `admin/lb-streams.spec.ts`.
...
- Tests: `ConnectionStoreTest` (seam on `lines_live` and Redis; an agent that does not answer), `ConnectionLimitsTest`, `ConnectionSnapshotTest` (digest, whole-snapshot apply, bad chunk), `LargeSnapshotChunkingTest` (20 chunks of 1000: nothing applied before the last, a bad chunk leaves the store unchanged), `ClusterApiTest` (kills as commands), `HlsReapingTest`, `ConnectionAdmissionTest`, `AgentAdmissionTest`; Go `registry_test.go` (the reaper too), `snapshot_test.go`.
...
- MAIN's downtime: the orphan purge counts silence from `max(last_seen_at, cluster_ready_at)` and orphans nobody while the fleet silence guard is up (`HlsReaping`). Tests: `MainOutageNoPurgeTest`.
