...
- `node.state` (P0: `certbot_ssl`, `governor`, `sysctl`) and `node.inventory` (the `cron:servers` inventory, on P1 since P2 is not built) through `Core/Cluster/NodeStateSink` once TELEMETRY is on. MAIN writes only those columns of the node's own row; `whitelist_ips`, `server_ip` and `status` are never taken from a node (`ClusterEventsTest`).
- `stream.progress` is not a separate event: `progress_info` rides `stream.state`, at most once a minute per stream from `cron:streams` (every 10 s for a created channel's encode), and P0 compaction keeps one state per row. It moves to the bus with the P2 lane.
...
- Divergence and touches: a CONNECTIONS node's divergence writers (`cron:users`, `fanout_sync`) spool each viewer's rate as a P1 `conn.divergence` (`Core/Cluster/DivergenceSink`), and MAIN writes `lines_divergence` for the node's own connections. The `events` op takes the P2 lane (no cursor, latest per key by the event's `t`) with `conn.touch`, kept on the cluster bus for nodes that reap their own HLS viewers and in MAIN's store otherwise, and hello and heartbeat advertise it as `p2_types` (ADR 0004, tenth Phase 6 increment, with the agent's contract). Tests: `ConnectionDivergenceTest`, `ConnectionTouchTest`.
