...
- **Not built yet:** changes to MAIN's HTTPS port, `server_ip` and `private_ip` are not announced (only `http_broadcast_port` and `cluster_api_port` bump `cluster_policy_ver`); E2E `admin/lb-telemetry.spec.ts`.
...
- **Not built yet:** the `artefact` op and the agent's artefact download with its hash refusal (`ArtefactHashRefusalTest`); a dedicated `HardModeKillChannelTest`; E2E `admin/lb-streams.spec.ts`.
...
- Tests: `ConnectionStoreTest` (seam on `lines_live` and Redis; an agent that does not answer), `ConnectionLimitsTest`, `ConnectionSnapshotTest` (digest, whole-snapshot apply, bad chunk), `ClusterApiTest` (kills as commands), `HlsReapingTest`, `ConnectionAdmissionTest`, `AgentAdmissionTest`; Go `registry_test.go` (the reaper too), `snapshot_test.go`.
