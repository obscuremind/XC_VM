...
- The cluster FPM pools: `Domain/Cluster/ClusterPool` writes `cluster_ctl` (control ops) and `cluster_ingest` (ingest ops) to `bin/php/etc/cluster/`, sized by the formulas in §3. `status` (boot, update, through `cluster:pools` as xc_vm) and `cron:servers` start, resize and revive them, always as xc_vm. The fixed `/cluster/v1/` location passes to the `cluster_ctl` and `cluster_ingest` upstreams, each with a panel pool as backup for a request that cannot reach the pool. Until both pools answer FPM's ping, `tmp/cluster_ready` is missing and every op but `health` gets a panel-signed `503 STARTING`; a reload keeps the marker. Tests: `ClusterPoolTest` (sizing, rendering, `ensure()`, the STARTING gate, the lane list in `nginx.conf`; opt-in against a real php-fpm).
- **Not built yet:** rendered nginx config (`ClusterNginxConfig`, `cluster_locations.conf`, `cluster.d/`; the old-port servers from `ClusterEndpoint` still pass to a panel pool), the `cluster_ctl` listen-queue input to the fleet silence guard, and the cluster bus's other uses (`ClusterBus` exists and carries the command and ack wake-ups and admission reservations; nonces, telemetry and semaphores are not on it yet, so each heartbeat still writes `cluster_nodes` and each authenticated request inserts into `cluster_nonces`); a fleet-wide `cluster:reenrol --all` (each node is re-enrolled with `server:enrol`); `HttpsRequiredRecoveryTest`; E2E `admin/cluster-nodes.spec.ts`.
...
- The `commands` long-poll runs on the `cluster_ctl` pool (Phase 2's `ClusterPool`), so a held long-poll no longer occupies a panel FPM worker. Two exceptions remain: agents that reach MAIN through the old-port servers (`cluster_legacy.conf` still passes to the panel pool), and a request that cannot reach `cluster_ctl`, which nginx passes to the panel pool.
