...
- **Not built yet:** from admission, the `adm` claim and the `conn_admit` op with `lb_offline_admission` (`ConnectionAdmission` reserves on the cluster bus, or without it in MAIN's Redis or `cluster_reservations`); TS closes from the fanout (`/events` carries only stream monitor events, so `FanoutSyncCommand` still closes daemon-served TS rows through MAIN's store); rebuilding the registry after an agent restart from anything but `registry.snap` (the fanout's `GET /connections` returns bare uuids); the touch on the bus (`conn.touch`; touches still go every 10 s as P0 upserts) and `conn.divergence`; `ConnectionIngestIdempotencyTest`, `MainOutageNoPurgeTest`, `LargeSnapshotChunkingTest` at 20k rows; E2E `admin/lb-playback-limits.spec.ts`; load tests.
