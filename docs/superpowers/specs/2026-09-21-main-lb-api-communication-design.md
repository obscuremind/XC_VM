...
- The cluster FPM pools: `Domain/Cluster/ClusterPool` writes `cluster_ctl` (control ops) and `cluster_ingest` (ingest ops) to `bin/php/etc/cluster/`, sized by the formulas in §3. `status` (boot, update) and `cron:servers` start, resize and revive them. The fixed `/cluster/v1/` location passes to the `cluster_ctl` and `cluster_ingest` upstreams, each with a panel pool as backup. Until both pools answer FPM's ping, `tmp/cluster_ready` is missing and every op but `health` gets a panel-signed `503 STARTING`. Tests: `ClusterPoolTest` (sizing, rendering, `ensure()`, the STARTING gate, the lane list in `nginx.conf`; opt-in against a real php-fpm).
...
- The `commands` long-poll runs on the `cluster_ctl` pool (Phase 2's `ClusterPool`), so a held long-poll no longer occupies a panel FPM worker.
